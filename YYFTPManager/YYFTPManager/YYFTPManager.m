...
//        NSLog(@"YYFTPRequest: The serverURL is illegal!");
    }
}
...
//        NSLog(@"YYFTPRequest: The localFileURL is illegal!");
    }
}

- (BOOL)start {
//    NSLog(@"YYFTPRequest: Need override by subclass!");
    return NO;
}
...
void resourceListReadStreamClientCallBack(CFReadStreamRef stream, CFStreamEventType type, void *clientCallBackInfo) {
    YYFTPManager *request = (__bridge YYFTPManager *)clientCallBackInfo;
...
- (BOOL)downloadFileRequestWithPath:(NSString *)path fileName:(NSString *)fileName {
    ...
    if (localFileURL.absoluteString.isValidateFileURLString) {
        if (self.localFileURL != localFileURL) {
...
//        NSLog(@"YYFTPRequest: The localFileURL is illegal!");
    ...
    if (self.serverURL == nil) {
//        NSLog(@"self.serverURL---%@",self.serverURL);
        return NO;
    }
    ...
    Boolean supportsAsynchronousNotification = CFReadStreamSetClient(self.readStream,
                                                                     ...
                                                                     &_streamClientContext);
//    NSLog(@"-----------downloadReadStreamClientCallBack----------");
